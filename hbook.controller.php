...
class hbookController extends hbook {
	...
	function init() {
        ...
        function procHbookContentWrite() {
			...
			$phonenum = $logged_info->phonenum[0]."-".$logged_info->phonenum[1]."-".$logged_info->phonenum[2];
...
*/
		...
		function procHbookTradeFinish() {
            // request 값을 모두 받음
            $obj = Context::getRequestVars();
			debugPrint($obj);

			$obj->ting++;
			$output = executeQuery("hbook.tradeBook", $obj);
		...
		function procHbookContentWish() {
			...
			//trade_srl 확인
			$check->trade_srl = Context::get('trade_srl');
		
			//wisher_nick 확인
			$check->wisher_nick = Context::get('wisher_nick');
				
			// wishinfo DB에서 해당 도서를 유저가 과거에 찜 했었는지 확인.
			$oHbookModel = &getModel('hbook');
			$output = $oHbookModel->getHbookWishCheck($check);
